...
<script>
  ...
  function showData() {
    $('#table-hasil-tes').DataTable({
      processing: true,
      serverSide: true,
      ajax: `<?= base_url()?>testoefl/getHasilTesToefl/<?= $id_tes?>`,
      responsive: {
          details: {
              type: 'column'
          }
        ...
        },
        {
          ...
          render: function(data, type, row) {
            ...
            <a href="javascript:void(0)" onclick="editPeserta('${row.id_peserta}')" class="me-1">
              ...
              <a href="<?= base_url()?>hasiltoefl/${row.id_peserta}" target="_blank" class="me-1">
  ...
  function editPeserta($id_peserta) {
    $.ajax({
      url: "<?= base_url()?>/testoefl/getPeserta/" + $id_peserta,
      type: "get",
  ...
  function simpanPesertaToefl(e) {
    ...
    $.ajax({
      url: "<?= base_url()?>/testoefl/simpanPesertaToefl",
      type: "POST",
      data: {
        id: id,
        nama: nama,
