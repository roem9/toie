      ...
      <div class="modal-body">
        ...
        <form id="formTambahTes">
          ...
          <div class="col-12 mb-3">
            ...
            <select name="id_soal" id="id_soal" class="multisteps-form__input form-control">
              <option value="">Pilih Soal</option>
              <?php $toefl = list_soal_toefl();
              foreach ($toefl as $toefl) : ?>
                <option value="<?= $toefl['id_soal'] ?>"><b><?= $toefl['nama_soal'] ?></b></option>
              <?php endforeach; ?>
            </select>
          </div>
          <div class="col-12 mb-3">
          ...
          <div class="form-group">
            ...
            <div class="mt-3">
              ...
              <p class='text-xs'>
                Tes TOEFL :<br>
                $nama = nama, $t4_lahir = Tempat Lahir, $tgl_lahir = Tgl Lahir, $alamat = Alamat, $no_wa = No. WA, $email = email, $jk = gender, $skor = Skor Tes, $tes = Nama Tes, $tgl_tes = Tanggal Tes, $tgl_pengumuman = Tanggal Pengumuman
              </p>
...
<script>
  ...
  function showData() {
    $('#table-tes').DataTable({
      processing: true,
      serverSide: true,
      ajax: `<?= base_url()?>/testoefl/getAllTes`,
      responsive: {
          details: {
              type: 'column'
          }
      ...
      columns: [
        ...
        },
        {
          ...
          render: function(data, type, row) {
            return `
            <a href="<?= base_url()?>testoefl/hasil/${row.tes}" class="me-1">
  ...
  function tambahTes(e) {
    ...
    let id_soal = $(`#formTambahTes [name='id_soal']`).val();
  ...
  function editTes($id_tes) {
    $.ajax({
      url: "<?= base_url()?>testoefl/getTes/" + $id_tes,
      type: "get",
      success: function(hasil) {
        var $obj = $.parseJSON(hasil);
        if ($obj.id_tes != '') {
          ...
          $(`#formTambahTes [name='id_soal']`).val($obj.id_soal);
  ...
  function hapusTes(id_tes, nama_tes) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: "<?= base_url()?>/testoefl/hapusTes/" + id_tes,
          type: "get",
          success: function(hasil) {
            if (result.isConfirmed) {
  ...
  function editStatusTes(id_tes, status, nama_tes) {
    $.ajax({
      url: "<?= base_url()?>/testoefl/editStatusTes",
      type: "POST",
      data: {
        id_tes: id_tes,
        status: status
